...
namespace :cron do
  ...
  task download_backup: :environment do
    path = "#{Rails.public_path.join('psu')}"
    ...
    response = HTTParty.get 'https://webservices.digitalmeasures.com/login/service/v4/SchemaData:backup/INDIVIDUAL-ACTIVITIES-University',
                            basic_auth: auth
    ...
    Dir.foreach(path) do |item|
      ...
      File.delete("#{path}/#{item}") if file_date < year_ago && File.exist?("#{path}/#{item}")
    end
  end
end
