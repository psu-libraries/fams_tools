class CvPresentationIntegrateJob < ApplicationJob
  ...
  def perform(params, log_path)
    ...
    error_logger.info "CV Presentations Integration to #{params[:target]} initiated at: #{DateTime.now}"
    ...
    error_logger.info "Errors for CV Presentations Integration to #{params[:target]} at: #{DateTime.now}"
