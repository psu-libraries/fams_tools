class GpaIntegrateJob < ApplicationJob
  ...
  def perform(params, log_path)
    ...
    error_logger.info "GPA Integration to #{params[:target]} initiated at: #{DateTime.now}"
    ...
    error_logger.info "Errors for GPA Integration to #{params[:target]} at: #{DateTime.now}"
