          ...
          xml.Record('username' => faculty.access_id) do
            ...
            faculty.contract_faculty_links.order('contract_id ASC').each do |link|
              ...
              xml.CONGRANT do
                ...
                if contract.title.present?
                  xml.TITLE_(link.contract.title.gsub(/[^[:print:]]/, ''),
                             access: 'READ_ONLY')
                end
                ...
                if %w[EM IST MD SC UC UL].include?(faculty.college)
                  if contract.effort_academic && !contract.effort_academic.zero?
                    xml.ACADEMIC_(contract.effort_academic,
                                  access: 'READ_ONLY')
                  end
                  if contract.effort_summer && !contract.effort_summer.zero?
                    xml.SUMMER_(contract.effort_summer,
                                access: 'READ_ONLY')
                  end
                  if contract.effort_calendar && !contract.effort_calendar.zero?
                    xml.CALENDAR_(contract.effort_calendar,
                                  access: 'READ_ONLY')
                  end
                end
                  ...
                  ContractFacultyLink.joins(:contract)
                    ...
                    if index == 0
                      ...
                      if amendment_contract.end_date
                        xml.DTM_END_ Date.strptime(amendment_contract.end_date.to_s, '%Y-%m-%d').strftime('%B'),
                                     access: 'READ_ONLY'
                        xml.DTD_END_ Date.strptime(amendment_contract.end_date.to_s, '%Y-%m-%d').strftime('%d'),
                                     access: 'READ_ONLY'
                        xml.DTY_END_ Date.strptime(amendment_contract.end_date.to_s, '%Y-%m-%d').strftime('%Y'),
                                     access: 'READ_ONLY'
                      end
                    end
                    xml.AMENDMENT do
                      ...
                      if amendment_contract.total_anticipated
                        xml.AMOUNT_ANTICIPATE_ amendment_contract.total_anticipated,
                                               access: 'READ_ONLY'
                      end
                      if amendment_contract.start_date
                        xml.DTM_START_ Date.strptime(amendment_contract.start_date.to_s, '%Y-%m-%d').strftime('%B'),
                                       access: 'READ_ONLY'
                        xml.DTD_START_ Date.strptime(amendment_contract.start_date.to_s, '%Y-%m-%d').strftime('%d'),
                                       access: 'READ_ONLY'
                        xml.DTY_START_ Date.strptime(amendment_contract.start_date.to_s, '%Y-%m-%d').strftime('%Y'),
                                       access: 'READ_ONLY'
                      end
                      if amendment_contract.end_date
                        xml.DTM_END_ Date.strptime(amendment_contract.end_date.to_s, '%Y-%m-%d').strftime('%B'),
                      ...
                      if amendment_contract.awarded
                        xml.DTM_AWARD_ Date.strptime(amendment_contract.awarded.to_s, '%Y-%m-%d').strftime('%B'),
                                       access: 'READ_ONLY'
                        xml.DTD_AWARD_ Date.strptime(amendment_contract.awarded.to_s, '%Y-%m-%d').strftime('%d'),
                                       access: 'READ_ONLY'
                        xml.DTY_AWARD_ Date.strptime(amendment_contract.awarded.to_s, '%Y-%m-%d').strftime('%Y'),
                                       access: 'READ_ONLY'
                      end
                    end
                ...
                if contract.start_date
                  xml.DTM_START_ Date.strptime(contract.start_date.to_s, '%Y-%m-%d').strftime('%B'),
                                 access: 'READ_ONLY'
                  xml.DTD_START_ Date.strptime(contract.start_date.to_s, '%Y-%m-%d').strftime('%d'),
                                 access: 'READ_ONLY'
                  xml.DTY_START_ Date.strptime(contract.start_date.to_s, '%Y-%m-%d').strftime('%Y'),
                                 access: 'READ_ONLY'
                end
                if !updated_end_date && contract.end_date
                  xml.DTM_END_ Date.strptime(contract.end_date.to_s, '%Y-%m-%d').strftime('%B'),
                               access: 'READ_ONLY'
                  xml.DTD_END_ Date.strptime(contract.end_date.to_s, '%Y-%m-%d').strftime('%d'),
                               access: 'READ_ONLY'
                  xml.DTY_END_ Date.strptime(contract.end_date.to_s, '%Y-%m-%d').strftime('%Y'),
                               access: 'READ_ONLY'
                end
                ...
                if contract.notfunded
                  xml.DTM_DECLINE_ Date.strptime(contract.notfunded.to_s, '%Y-%m-%d').strftime('%B'),
                                   access: 'READ_ONLY'
                  xml.DTD_DECLINE_ Date.strptime(contract.notfunded.to_s, '%Y-%m-%d').strftime('%d'),
                                   access: 'READ_ONLY'
                  xml.DTY_DECLINE_ Date.strptime(contract.notfunded.to_s, '%Y-%m-%d').strftime('%Y'),
                                   access: 'READ_ONLY'
                end
              end
            end
