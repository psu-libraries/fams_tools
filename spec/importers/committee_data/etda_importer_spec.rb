...
RSpec.describe CommitteeData::EtdaImporter do
  ...
  let(:facultyOne)    { create :faculty, access_id: "mpk6156" }
  let(:facultyTwo)    { create :faculty, access_id: "aez1236" }


  let(:client)     { instance_double(Etda::CommitteeRecordsClient) }
  let(:committees) { double("committees") }
...
context "when an error occurs" do
    before { allow(Faculty).to receive(:find_each).and_yield(faculty) }

    it "rescues CommitteeRecordsClientError and logs it" do
      allow(importer).to receive(:import_for_faculty)
      ...
      expect(Rails.logger).to have_received(:error).with(/abc123.*API down/)
    end
  end
end