...
RSpec.describe OspXMLBuilder do
  ...
  let(:data_sets) do
    [{:osp_key => 123456, :title => 'Title', :status => 'Pending', :submitted => "2016-01-01",
      ...
      :role => 'Principal Investigator', :pct_credit => 100, :id_number => 123},
      {:osp_key => 654321, :title => 'Title', :status => 'Pending', :submitted => "2016-01-01",
      :awarded => '', :requested => 1000, :funded => '', :totalanticipated => '', :startdate => '',
      ...
      :role => 'Principal Investigator', :pct_credit => 100, :id_number => 321}]
  end

  let(:osp_xml_builder_obj) {OspXMLBuilder.new}

  describe '#batched_osp_xml' do
    it 'should return batches of CONGRANT records in xmls' do
      ...
      expect(osp_xml_builder_obj.batched_osp_xml).to eq(
        '<?xml version="1.0" encoding="UTF-8"?>
          <Data>
            ...
            <Record username="xuz14">
              <CONGRANT>
                <OSPKEY access="LOCKED">133248</OSPKEY>
                <BASE_AGREE access="LOCKED"/>
                <TYPE access="LOCKED"/>
                <TITLE access="LOCKED">HCC: Small:  Supporting Synchronous Collaborative Sense-Making in Distributed Teams with Visualization</TITLE>
                <SPONORG access="LOCKED">National Science Foundation</SPONORG>
                <AWARDORG access="LOCKED">Federal Agencies</AWARDORG>
                <CONGRANT_INVEST>
                  <FACULTY_NAME>252775</FACULTY_NAME>
                  <FNAME>Xiaolong</FNAME>
                  <MNAME/>
                  <LNAME>Zhang</LNAME>
                  <ROLE>Principal Investigator</ROLE>
                  <ASSIGN>50</ASSIGN>
                </CONGRANT_INVEST>
                <AMOUNT_REQUEST access="LOCKED">498096</AMOUNT_REQUEST>
                <AMOUNT_ANTICIPATE access="LOCKED">0</AMOUNT_ANTICIPATE>
                <AMOUNT access="LOCKED">0</AMOUNT>
                <STATUS access="LOCKED">Not Funded</STATUS>
                <DTM_SUB access="LOCKED">July</DTM_SUB>
                <DTD_SUB access="LOCKED">31</DTD_SUB>
                <DTY_SUB access="LOCKED">2012</DTY_SUB>
                <DTM_AWARD/>
                <DTD_AWARD/>
                <DTY_AWARD/>
