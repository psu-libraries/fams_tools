...
describe PostPrintAnalyzer do
  ...
  before do
    ...
    allow(analyzer).to receive(:f_path).and_return("#{Rails.root.join('spec/fixtures/post_prints/post_print_list.csv')}")
    allow(analyzer).to receive(:post_prints_directory).and_return("#{Rails.root.join('spec/fixtures/post_prints')}")
    allow(analyzer).to receive(:analysis_directory).and_return("#{Rails.root.join('spec/fixtures/post_prints')}")
  ...
  describe 'analyze' do
    context 'when no error is returned from system call' do
      it 'does not write to error to File' do
        ...
        analyzer.analyze
        expect(File.read("#{Rails.root.join('spec/fixtures/post_prints/errors.txt')}")).to eq ''
      end
    end
    ...
    context 'when an error is returned from system call' do
      it 'writes error to File' do
        ...
        analyzer.analyze
        expect(File.read("#{Rails.root.join('spec/fixtures/post_prints/errors.txt')}")).to eq "abc123/intellcont/test.pdf\n"
      end
    end
  end
end
...
def clear_directory
  Dir.foreach("#{Rails.root.join('spec/fixtures/post_prints')}") do |f|
    fn = Rails.root.join(f).to_s
